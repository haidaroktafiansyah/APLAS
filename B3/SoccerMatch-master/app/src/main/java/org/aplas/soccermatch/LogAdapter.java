...
public class LogAdapter extends RecyclerView.Adapter<LogAdapter.LogViewHolder> {
    ...
    public void onBindViewHolder(final LogViewHolder holder, final int listPosition) {
        ...
        Uri homeUri = Uri.parse(itemList.get(listPosition).getImage());
        InputStream imageStream1 = null;
        try {
            imageStream1 = activity.getContentResolver().openInputStream(homeUri);
        ...
        if (name.startsWith("Goal")) {
            holder.img.setImageBitmap(selectedImage1);
